...
<<>>=
plot_images(image_array = x_train, y = mnist$train$y, idx = c(1:2))
@
...
\subsection{1.3}
The reason there is 312 parameters in the first layer is due to the filter and filter size. We have 32 filters  where the filter size or kernel\_size is $ 3\times 3$. Hence the number of parameters becomes $(3 \times 3 + 1) \times 32 = 10 \times 32 = 320$  where the additional 1 is the bias. 
...
Here it stopped early. TThe bets results are from epoch 4.
...
@
\subsection{1.5}
Now we test different hyper parameter values. The three hyper parameter i have chosen are the kernel size for the convulutional layers, the learning rate for the optimizer,  and the Weight decay coefficient for the $L_2$  regularizer. 
...
The full results for the model are the following:
...
knitr::include_graphics("cifar_model")
@
...
<<>>=
plot_images_2(idx = c(1337, 1338), x_train, y_train)
@
\subsection{task 1.8}
...
\subsection[task 1.9]
First we start of by adding more hidden layers, both convulutional and dense layers,  and increasing the number of hidden uni, which increases the representational capacity of the model. This is so we can potentially overfit. But due to computational reasons i will not go all out but simply try adding a relatively small bit. 
We also increase the epochs to 15.
...
knitr::include_graphics("cifar_model_2")
@
...
For the next model, we increase the batch size, since a batch with more samples results in more informative gradients with lower variance, while also being more computationally efficient. We also add implicit zero padding to keep the representation size large while also increasing the convolution kernel width to increase the number of parameters in the model . Furthermore, as inspired by the linked guide on the tensorflow webpage for this data set. we can add \texttt{layer_activation_leaky_relu(0.1)} which will allow a small, non-zero gradient when the input is negative and is supposed to helps prevent neurons becoming inactive and step learning while training. We also increase the number of epochs little bit again and I will tune the learning rate manually as to lower it after 10 epochs, since a improper learning rate will result in a model with low effective capacity.
...
<<eval=FALSE>>=
Epoch 30/30
98/98 [==============================] - 73s 748ms/step - loss: 0.9004 - accuracy: 0.7981 - val_loss: 0.9849 - val_accuracy: 0.7628 - lr: 1.3534e-04
@
